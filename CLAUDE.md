...
> **Читать первым: `SPEC.md`.** Этот репозиторий — прямой наследник collimator (та же история
> git, те же скрипты, агенты и инструменты); назначение изменилось: компилятор не строится,
> добавляется слой содержания — профили типов документов, эталоны, каталог типов. Всё ниже —
> правила collimator для скриптов и инструментов, они остаются в силе; расхождение со SPEC.md
> решается в пользу SPEC.md.
